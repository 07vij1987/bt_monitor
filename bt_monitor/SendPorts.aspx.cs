    ...
    public partial class SendPorts : System.Web.UI.Page
    {
        ...
        protected void grdViewSendPorts_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "ShowPopup")
                {
                    ...
                    DataSet SendPortInfo = bt_helper.dbHelper.ExecuteMgmtQueryDataSet(ConfigurationManager.AppSettings["serverNameKey"].ToString(), strcommand);
        ...
        private void LoadSendPortInfo(DataSet portInfo)
        {
            ...
            if (portInfo.Tables[6].Rows.Count > 0)
            {
                ...
                {
                    lblFilters.Text=bt_helper.dbHelper.ReadFilters(portInfo.Tables[6].Rows[0][0].ToString());
                }
            }
            #endregion
        }
        ...
        private void ManageSendPort(ArrayList portNameList, string level)
        {
            ...
            {               
                ...
                if (portNameList != null)
                {
                    ...
                    {
                        bt_helper.dbHelper.ManageSendPortOM(port, ConfigurationManager.AppSettings["serverNameKey"].ToString(), portStatus);
                    }
                }
            }
            catch (Exception ex)
