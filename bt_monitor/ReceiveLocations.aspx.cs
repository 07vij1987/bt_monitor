    ...
    public partial class ReceiveLocations : System.Web.UI.Page
    {
        ...
        protected void grdViewReceiveLocations_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "ShowPopup")
                {
                    ...
                    ReceivePipelineDataSet = bt_helper.dbHelper.ExecuteMgmtQueryDataSet(ConfigurationManager.AppSettings["serverNameKey"].ToString(), strcommand);
                    SendPipelineDataSet = bt_helper.dbHelper.ExecuteMgmtQueryDataSet(ConfigurationManager.AppSettings["serverNameKey"].ToString(), sendPipelineCmd);
        ...
        private void LoadGrid()
        {
            ...
            try
            {
                ...
                DataTable receiveLocation = bt_helper.dbHelper.ExecuteMgmtQuery(ConfigurationManager.AppSettings["serverNameKey"].ToString(), strcommand);
        ...
        private void BindFilterList()
        {
            ...
            try
            {
                ...
                dt = bt_helper.dbHelper.ExecuteMgmtQueryDataSet(ConfigurationManager.AppSettings["serverNameKey"].ToString(), strcommand);
        ...
        private void ManageReceiveLocation(ArrayList locationNameList, bool status)
        {
            try
            {
                ...
                if (locationNameList != null)
                {
                    ...
                    {
                        bt_helper.dbHelper.ManageReceiveLocationOM(recvLocation, portNameList[portCount].ToString(), ConfigurationManager.AppSettings["serverNameKey"].ToString(), status);
        ...
        private void EnableDisableServiceWindow(ArrayList locationNameList, bool status)
        {
            try
            {
                if (locationNameList != null)
                {
                    ...
                    foreach (string recvLocation in locationNameList)
                    {
                        bt_helper.dbHelper.ManageReceiveLocationWindowOM(recvLocation, portNameList[portCount].ToString(), ConfigurationManager.AppSettings["serverNameKey"].ToString(), status);
                        portCount = portCount + 1;
                    }
                }
            }
